...
*   **a.** Convert the binary number 1000100111000011 to hexadecimal and octal.
    ...
    *   **Octal:** Divide the binary number into groups of 3 bits from right to left: `001 000 100 111 000 011`. Convert each group to its octal equivalent: `1 0 4 7 0 3`. The octal representation is `104703`.
*   **b.** Using parity coding, convert 67 (base 10) to odd parity.
    *   First, convert 67 to binary: `1000011`. Since we are using a word length of 1 byte, we'll represent it with 8 bits.
    *   **Odd Parity:** Count the number of 1s in the binary representation (`01000011`). There are three 1s. For odd parity, we leave it as is, since we have an odd number of 1s.
